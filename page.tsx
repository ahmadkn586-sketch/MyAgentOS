...
export default function CallsPage() {
  ...
  const getStatus = (call: any) => {
    ...
    if (summary.includes('emergency')) return { label: 'Emergency Forwarded', color: 'bg-red-50 text-danger' }
    if (summary.includes('scam') || summary.includes('spam') || summary.includes('robocall')) return { label: 'Scammer Blocked', color: 'bg-gray-100 text-gray-600' }
    if (summary.includes('appointment') || summary.includes('booked')) return { label: 'Appointment Booked', color: 'bg-green-50 text-success' }
      ...
      <main className="flex-1 p-10">
        ...
        {!vapiKey && !loading && (
          <div className="border border-bordergray rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">Your calls will appear here once Vapi is connected.</p>
            <Link href="/settings" className="bg-electric text-white px-5 py-2.5 rounded-lg text-sm font-semibold">Go to Settings</Link>
          </div>
        )}
            ...
            <div className="flex gap-2 mb-6">
              {['all', 'Handled', 'Emergency Forwarded', 'Scammer Blocked', 'Appointment Booked'].map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium ${filter === f ? 'bg-electriclight text-electric' : 'bg-gray-50 text-gray-600'}`}
                >
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredCalls.length === 0 && !loading && (
                    <tr><td colSpan={6} className="text-center py-12 text-gray-400">No calls yet. Once your AI handles calls, they'll appear here.</td></tr>
                  )}
                  {filteredCalls.map((call) => {
                    ...
                    return (
                      <tr key={call.id} className="border-t border-bordergray">
                        ...
                        <td className="px-4 py-3 font-medium">{call.customer?.number || 'Unknown'}</td>
          ...
          <div className="bg-white w-full max-w-md h-full overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-gray-500">{new Date(selected.startedAt).toLocaleString()} · {formatDuration(selected.startedAt, selected.endedAt)}</p>
              </div>
            ...
            <div className="space-y-3">
              {(selected.transcript || []).map((msg: any, i: number) => (
                <div key={i} className={`flex ${msg.role === 'assistant' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'assistant' ? 'bg-electriclight text-electric' : 'bg-gray-100'}`}>
